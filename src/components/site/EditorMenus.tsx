...
import { ChevronDown, ChevronUp, Eye, EyeOff, Plus, Star, Trash2 } from 'lucide-react';
import { Site, type ItemMenuAdmin, type MenuAdmin } from '../../services/site';
...
export const EditorMenus: React.FC<Props> = ({ menus, onMenusAlterados, showToast }) => {
...
const ItemLinha: React.FC<{
  ...
  onExcluir: () => void;
}> = ({ item, primeiro, ultimo, destaqueDisponivel, onMover, onSalvar, onExcluir }) => {
  const [rotulo, setRotulo] = useState(item.rotulo);
  const [destino, setDestino] = useState(item.destino);

  return (
    <li className="flex flex-wrap items-center gap-2 border border-slate-200 rounded-xl p-2.5">
